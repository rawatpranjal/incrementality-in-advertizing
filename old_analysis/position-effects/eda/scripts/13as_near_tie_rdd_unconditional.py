...
import numpy as np
import pandas as pd
# tqdm removed for non-interactive use
...
def fit_lpm_local_linear(y: np.ndarray, lucky: np.ndarray, z: np.ndarray, cluster: np.ndarray = None):
    ...
    try:
        ...
        # Design matrix: [1, lucky, z, z*lucky]
        z_lucky = z * lucky
...
def main():
    ...
    out = RESULTS_DIR / f"13as_near_tie_rdd_unconditional_{args.round}.txt"
    ...
    with open(out, 'w') as fh:
        ...
        if all_results:
            ...
            # Table 1: Main results at τ = 0.02 (unconditional clicked)
            w("\n--- Table 1: Exposure and Click Decomposition by Rank Boundary (τ = 0.02) ---")
            w("(Unconditional outcomes as primary specification)")
            w("")
            w(f"{'Boundary':<12} {'Δ_exp':<12} {'(SE)':<10} {'Δ_click':<12} {'(SE)':<10} {'N pairs':<10}")
            w("-" * 66)
            ...
            w("")
            w(f"{'τ':<10} {'Δ_exp':<12} {'(SE)':<10} {'p-value':<10} {'N pairs':<10} {'Balance':<10}")
            w("-" * 62)
            ...
            # Save full results
            csv_out = RESULTS_DIR / f"13as_near_tie_rdd_unconditional_{args.round}_full.csv"
