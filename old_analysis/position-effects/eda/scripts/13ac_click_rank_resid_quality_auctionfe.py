...
def main():
    ap = argparse.ArgumentParser(description='Residualize log(rank) on log(quality) with AUCTION FE; test residual in GLM with QUALITY offset')
    ...
    # First-stage within (AUCTION_ID) — common slope beta_within, auction-specific intercepts
    lr = safe_log_arr(df['RANKING'].values)
    lq = safe_log_arr(df['QUALITY'].values)
    gm_lr = df.groupby('AUCTION_ID')['RANKING'].transform(lambda s: np.log(np.clip(s.values.astype(float), 1e-12, None)).mean()).values
    gm_lq = df.groupby('AUCTION_ID')['QUALITY'].transform(lambda s: np.log(np.clip(s.values.astype(float), 1e-12, None)).mean()).values
    lr_til = lr - gm_lr
    lq_til = lq - gm_lq
    denom = float(np.dot(lq_til, lq_til))
    beta_within = float(np.dot(lq_til, lr_til) / denom) if denom > 0 else 0.0
    # Residual r = lr_til - beta * lq_til (equiv to lr - (alpha_auc + beta*lq))
    r = lr_til - beta_within * lq_til

    # Second-stage GLM with QUALITY offset and residual as regressor
    ...
    print(f"\n=== {args.round.upper()} Rank residual beyond quality with AUCTION FE (first-stage) ===")
    print(f"rows={len(df):,} auctions={df['AUCTION_ID'].nunique():,} impressions={int(df['imps'].sum()):,} clicks={int(df['clks'].sum()):,}")
    print(f"Within-slope beta(log(rank)~log(quality) | auction FE) = {beta_within:.4f}")
