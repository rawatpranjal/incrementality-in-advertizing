...
OUTPUT_FILE = RESULTS_DIR / "05_catalog_nlp_eda.txt"
...
def main():
    ...
    with open(OUTPUT_FILE, 'w') as f:
        log("=" * 80, f)
        log("CATALOG NLP EDA: TEXT MINING ANALYSIS", f)
        log("=" * 80, f)
        log("", f)
        ...
        if unique_brands > 0:
            ...
            for brand in top20_brands:
                brand_prices = df[df['brand'] == brand]['CATALOG_PRICE'].dropna()
        ...
        for color in top15_colors:
            ...
            color_prices = df[color_mask]['CATALOG_PRICE'].dropna()
        ...
        valid_price = df['CATALOG_PRICE'].notna() & (df['CATALOG_PRICE'] > 0)
        log(f"Products with valid price (>0): {valid_price.sum():,} ({100*valid_price.mean():.2f}%)", f)
        log("", f)

        # Price distribution
        prices = df[valid_price]['CATALOG_PRICE']
        ...
        if len(price_name_data) > 0:
            corr_price_name = price_name_data['CATALOG_PRICE'].corr(price_name_data['name_char_count'])
            corr_price_name_word = price_name_data['CATALOG_PRICE'].corr(price_name_data['name_word_count'])
        ...
        if len(price_desc_data) > 0:
            corr_price_desc = price_desc_data['CATALOG_PRICE'].corr(price_desc_data['desc_char_count'])
            corr_price_desc_word = price_desc_data['CATALOG_PRICE'].corr(price_desc_data['desc_word_count'])
        ...
        price_with_brand = df[valid_price & df['brand'].notna()]['CATALOG_PRICE']
        price_without_brand = df[valid_price & df['brand'].isna()]['CATALOG_PRICE']
        ...
        zero_price = (df['CATALOG_PRICE'] == 0).sum()
        negative_price = (df['CATALOG_PRICE'] < 0).sum()
        ...
        top_prices = df.nlargest(10, 'CATALOG_PRICE')[['PRODUCT_ID', 'NAME', 'CATALOG_PRICE', 'brand']]
        for _, row in top_prices.iterrows():
            name_trunc = str(row['NAME'])[:60] if pd.notna(row['NAME']) else 'N/A'
            log(f"  ${row['CATALOG_PRICE']:,.0f}: {name_trunc}... (brand: {row['brand']})", f)
        log("", f)
        ...
        # =================================================================
        log("=" * 80, f)
        log("SECTION 10: SUMMARY STATISTICS", f)
        log("-" * 40, f)
        log("", f)
        ...
        for metric, value in scorecard:
            log(f"  {metric}: {value}", f)
        log("", f)

        log("KEY FINDINGS:", f)
        log("-" * 40, f)
        log(f"  1. NAME field is highly populated ({100*name_valid/total:.1f}% coverage)", f)
        log(f"  2. NAME often equals DESCRIPTION ({100*name_eq_desc/total:.1f}% identical)", f)
        log(f"  3. Brand extraction rate: {100*brand_count/total:.1f}% ({unique_brands:,} unique brands)", f)
        log(f"  4. Brand concentration Gini: {gini:.3f} (higher = more concentrated)", f)
        log(f"  5. Color coverage: {100*color_count/total:.1f}%, Size coverage: {100*size_count/total:.1f}%", f)
        log(f"  6. TF-IDF vocabulary: {vocab_size_uni:,} unigrams, {vocab_size_bi:,} bigrams", f)
        log(f"  7. Price range: ${prices.min():.0f} to ${prices.max():,.0f} (median ${prices.median():.0f})", f)
        log("", f)
