...
def print_time_aggregation(results, outcome):
    """Print time (calendar) aggregation."""
    time_agg = results['time']

    print(f"Time Aggregation for {outcome}:")
    print("-" * 60)
    print(f"{'Time':<8} {'ATT':<15} {'SE':<12} {'Sig?':<6}")
    print("-" * 60)

    for _, row in time_agg.iterrows():
        t = row['time']
        att = row['TimeAggregation_ATT']
        se = row['TimeAggregation_analytic_std_error']
        sig = row['TimeAggregation_pointwise conf. band_zero_not_in_cband']
        sig_str = "***" if sig == '*' else ""

        if outcome == 'total_gmv':
            print(f"t={int(t):<5}  ${att:+.2f}{'':>8} ${se:.2f}{'':>6} {sig_str}")
        else:
            print(f"t={int(t):<5}  {att:+.6f}{'':>4} {se:.6f} {sig_str}")
    print()
